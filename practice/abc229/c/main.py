...
N,W = MI()
cheese = [LI() for _ in range(N)]

# 美味しさ順に並び替える
cheese_sort = sorted(cheese,reverse=True)

ans = 0
weight = 0
for i in range(N):
    if weight < W:
        if (W-weight) > cheese_sort[i][1]:
            ans += cheese_sort[i][0] * cheese_sort[i][1]
            weight += cheese_sort[i][1]
        else:
            ans += cheese_sort[i][0] * (W-weight)
            weight = W
    else:
        break
print(ans)
